...
buffer_atual = 0
qualidade_atual = "HIGH"       # NOVO: Estado global da qualidade
ultimo_t_download = 0.0        # NOVO: Memória do tempo do último bloco
lock = threading.Lock()
executando = True
...
def main():
    ...
    # Aqui o script lê as ordens enviadas pelo arquivo 'topology.py'
    # =====================================================================
    ...
    # =====================================================================
    BUFFER_MAX = 50           # Limite Físico: Tamanho máximo da RAM alocada
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                while executando and (time.time() - tempo_inicio) < args.duration:
                    ...
                    # Lê o número de sequência (os 8 primeiros bytes que o servidor mandou)
                    header = s.recv(8).decode()
                    seq_recebido = int(header)
                    
                    if seq_recebido > seq_esperado:
                        gravar_log('Perda de Rede (TCP-Wait)', qualidade_atual, ocupacao)
                    ...
                    # --- 5.4: A MÁGICA: TOMADA DE DECISÃO DE QUALIDADE (ABR) ---
                    ...
                    # Regra de Otimismo: Download rápido E buffer seguro
