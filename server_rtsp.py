...
def main():
    ...
    HOST = '0.0.0.0'
    PORT_RTSP = args.port
    PORT_RTP = args.port + 1 # RTP convencionalmente usa a porta adjacente superior
    
    # =====================================================================
    # BLOCO 2: PERFIL DA MÍDIA (UDP PUSH)
    # Define o tamanho do pacote para evitar fragmentação no IP (MTU)
    # =====================================================================
    CHUNK_SIZE = 1316 # tamanho padrão da industria 
    ...
    def canal_controle_tcp():
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s_tcp:
            ...
            # Descobre o IP do cliente dinamicamente ao conectar no TCP
            ...
            with conn:
                while True:
                    ...
                    # Altera o estado global da máquina com base no comando
                    if "PAUSE" in cmd:
                        estado_transmissao = "PAUSE"
    ...
    def envio_dados_udp():
        global estado_transmissao, cliente_addr
        seq_num = 0  # Contador de sequência do RTP
    ...
    try:
        while True: time.sleep(1) # Mantém o processo pai vivo
